...
The run card thumbnail (`src/components/left-panel/run-card/run-thumbnail.tsx`) draws a mini-map: a baked
base map PNG from `src/assets/basemap-thumbs/` with an optional SST layer, then the storm track and
pressure system markers on top. The PNGs are Web Mercator crops of an exact lat/lng window, generated by
`scripts/gen-basemap-thumbs.py`, which fetches the same Esri tile layers the live app uses (satellite,
street, relief), stitches the tiles, and crops them.

```bash
pip install Pillow
python3 scripts/gen-basemap-thumbs.py
```
