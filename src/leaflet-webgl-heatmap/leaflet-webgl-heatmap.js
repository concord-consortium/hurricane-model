...
const LeafletWebGLHeatmap = L.Renderer.extend({
  ...
  draw: function() {
    ...
    if (dataLen) {
      ...
      for (var i = 0; i < dataLen; i++) {
        var dataVal = data[i],
          latlng = L.latLng(dataVal.slice(0, 2)),
          point = map.latLngToContainerPoint(latlng);

        heatmap.addPoint(
          floor(point.x),
          floor(point.y),
          scaleFn(latlng, dataVal.length >= 4 ? dataVal[3] : this.size),
          dataVal[2]
        );
      }
...
export default LeafletWebGLHeatmap;
