...
export const HurricaneTrack = observer(function HurricaneTrack() {
  ...
  const segments = React.useMemo(() => buildSegments(hurricaneTrack), [trackLength]);
    ...
    <Pane name="selectedTrack" style={{ zIndex: 430 }}>
      {segments.map((segment, idx) =>
        <Polyline key={`${idx}-border`} className={css.hurricaneTrackBorder}
          positions={segment.positions} weight={7} />
      )}
      {lastPoint &&
        <Polyline key="tail-border" className={css.hurricaneTrackBorder}
          positions={tail} weight={7} />}
      {segments.map((segment, idx) =>
        <Polyline key={`${idx}`} className={css.segment + " " + css["segmentCategory" + segment.category]}
          pane="shadowPane" positions={segment.positions} weight={5} />
      )}
      {lastPoint &&
        <Polyline key="tail" className={css.segment + " " + css["segmentCategory" + lastPoint.category]}
          pane="shadowPane" positions={tail} weight={5} />}
    </Pane>
  );
});
