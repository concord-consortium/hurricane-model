...
export function DraggableMapIcon({ children, dataTest, dimmed, disabled, label }: IProps) {
  const conditionalClasses = { [css.disabled]: disabled, [css.dimmed]: dimmed, [css.glow]: config.mode === "storm" };
  return (
    <div className={clsx(css.draggableMapIcon, conditionalClasses)} data-test={dataTest ?? "draggable-map-icon"}>
