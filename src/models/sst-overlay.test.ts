...
function makeSSTOverlay() {
  const simulation = new SimulationModel();
  return { overlay: new SSTOverlayModel(simulation), simulation };
}

function makePng(width: number, height: number, baseTempColor?: string) {
  const png = new PNG({ width, height });
  const c = rgb(baseTempColor ?? temperatureScale(20, "default"));
...
it("produces a data-URL when an anomaly is active", () => {
  const { overlay, simulation } = makeSSTOverlay();
